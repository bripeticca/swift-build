...
@Suite fileprivate struct MessageSerializationTests {
    ...
    @Test func buildOperationMessages() {
        ...
        assertMsgPackMessageRoundTrip(BuildOperationDiagnosticEmitted(kind: .error, location: .path(Path("/foo"), line: 12, column: 288), message: "something went wrong", sourceRanges: [], fixIts: [], traits: Set(), attachments: [:], childDiagnostics: []))
        assertMsgPackMessageRoundTrip(BuildOperationDiagnosticEmitted(kind: .error, location: .path(Path("/foo"), line: 12, column: 288), message: "something went wrong", locationContext: .target(targetID: 42), sourceRanges: [], fixIts: [], traits: Set(), attachments: [:],  childDiagnostics: []))
        assertMsgPackMessageRoundTrip(BuildOperationDiagnosticEmitted(kind: .error, location: .path(Path("/foo"), line: 12, column: 288), message: "something went wrong", locationContext: .task(taskID: 42, taskSignature: .taskIdentifier("task-id"), targetID: 42), appendToOutputStream: true, sourceRanges: [], fixIts: [], traits: Set(), attachments: [:],  childDiagnostics: []))
        assertMsgPackMessageRoundTrip(BuildOperationDiagnosticEmitted(kind: .error, location: .path(Path("/foo"), line: 12, column: 288), message: "something went wrong", locationContext: .globalTask(taskID: 42, taskSignature: .taskIdentifier("task-id-2")), appendToOutputStream: true, sourceRanges: [], fixIts: [], traits: Set(), attachments: [:],  childDiagnostics: []))
